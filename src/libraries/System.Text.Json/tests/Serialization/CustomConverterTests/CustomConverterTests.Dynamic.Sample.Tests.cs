...
namespace System.Text.Json.Serialization.Tests
{
    public static partial class CustomConverterTests
    {
        ...
        public static void VerifyPrimitives()
        {
            ...
            Assert.IsType<JsonValue>(obj.MyString);
            Assert.Equal("Hello", obj.MyString.Value);
            ...
            Assert.IsType<JsonValue>(obj.MyBoolean);
            Assert.True(obj.MyBoolean.Value);

            // Numbers must specify the type through a cast or assignment.
            Assert.IsType<JsonValue>(obj.MyInt);
        ...
        public static void VerifyArray()
        {
            ...
            obj.MyArray[0] = new JsonValue(10);
        ...
        public static void JsonDynamicTypes_Serialize()
        {
            ...
            dynamic dynamicString = new JsonValue(GuidJson, options);
            ...
            // char (string)
            dynamicString = new JsonValue("a", options);
            ...
            {
                dynamic dynamicNumber = new JsonValue(doc.RootElement, options);
            ...
            // Boolean
            dynamic dynamicBool = new JsonValue(true, options);
            Assert.True(dynamicBool.Value);
            json = JsonSerializer.Serialize(dynamicBool, options);
            Assert.Equal("true", json);

            // Array
            dynamic arr = new JsonArray(options);
            arr.Add(new JsonValue(1));
            arr.Add(new JsonValue(2));
        ...
        public static void JsonDynamicTypes_Deserialize()
        {
            ...
            JsonSerializer.Deserialize<JsonNode>("{}", options);
            JsonSerializer.Deserialize<JsonArray>("[]", options);
            JsonSerializer.Deserialize<JsonValue>("true", options);
            JsonSerializer.Deserialize<JsonValue>("0", options);
            JsonSerializer.Deserialize<JsonValue>("1.2", options);
            JsonSerializer.Deserialize<JsonObject>("{}", options);
            JsonSerializer.Deserialize<JsonValue>("\"str\"", options);
        }
        ...
        public static void JsonDynamicTypes_Deserialize_AsObject()
        {
            ...
            Assert.IsType<JsonValue>(JsonSerializer.Deserialize<object>("true", options));
            Assert.IsType<JsonValue>(JsonSerializer.Deserialize<object>("0", options));
            Assert.IsType<JsonValue>(JsonSerializer.Deserialize<object>("1.2", options));
            Assert.IsType<JsonObject>(JsonSerializer.Deserialize<object>("{}", options));
            Assert.IsType<JsonValue>(JsonSerializer.Deserialize<object>("\"str\"", options));
        }
        ...
        public static void VerifyMutableDom_WithoutUsingDynamicKeyword()
        {
            for (long l = 0; l < 1000; l++)
            {
                ...
                JsonObject obj = (JsonObject)JsonSerializer.Deserialize<object>(DynamicTests.Json, options);
                ...
                // Modify number elements.
                var arr = (JsonArray)obj["MyArray"];
                var elem = (JsonValue)arr[0];
                elem.Value = elem.GetValue<int>() + 1;
                elem = (JsonValue)arr[1];
                elem.Value = elem.GetValue<int>() + 1;
        ...
        [Fact]
        public static void VerifyNewDom_WithoutUsingDynamicKeyword()
        {
            var options = new JsonSerializerOptions();
            options.EnableDynamicTypes();
            ...
            var jObject = new JsonObject(options)
            {
                ...
                ["MyArray"] = new JsonArray(options) {2, 3, 42},
        ...
        [Fact]
        public static void VerifyNewDom_Explicit_WithoutUsingDynamicKeyword()
        {
            var options = new JsonSerializerOptions();
            options.EnableDynamicTypes();
        ...
        public static void DynamicObject_MissingProperty()
        {
            ...
            // We can detect missing, however:
            bool found = ((IDictionary<string, object>)obj).TryGetValue("NonExistingProperty", out object _);
            Assert.False(found);

            // without casting to IDictionary:
            found = obj.TryGetValue("NonExistingProperty", out object _);
            Assert.False(found);
        }
